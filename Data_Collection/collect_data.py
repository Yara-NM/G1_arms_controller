...
def main():
    ...
    th_video = threading.Thread(target=frame_writer_thread, 
                                args=(cap, video_path, ts_csv_path, stop_event), 
                                daemon=True)
    th_state = threading.Thread(target=state_logger_thread, 
                                args=(g1, state_csv, gripper , 
                                      stop_event), daemon=True)
    th_task = threading.Thread(target=task_runner_thread, 
                               args=(g1, REF_POINTS_JSON,  stop_event, gripper),daemon=True)
